...
class Application(Frame):
    ...
    def __init__(self,master):
        ...
        self.grid()
        self.create_widgets()
        self.notebook.focus() #eliminates the focus switch to first widget in frames when changing tabs
    ...
    def create_widgets(self):
        ...
        self.activeHelpFrame.grid(row=1,column=1,padx=5,pady=5,sticky=NW)
        self.textActiveHelp = Text(self.activeHelpFrame,wrap=WORD,height=11,width=40)
        ...
        self.activeReference = Label(self.activeHelpFrame,text="Reference",fg="blue",cursor="hand2")
    ...
    def populateThresholdSettings(self):
        ...
        if self.intSettings['thresholdMethod'].get() == 1:
            ...
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"The method determines the threshold that maximizes the 'total variance of levels'; equation 12 in reference. Performs poorly when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            self.thresholdLink = r"http://dx.doi.org/10.1109/TSMC.1979.4310076"
            Label(self.thresholdSettingsFrame,text="     No addtional settings needed,",fg='red').pack(anchor=W)
            ...
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"An iterative method to minimize cross-entropy of the gray and binarized image. Performs poorly when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            self.thresholdLink = r"http://dx.doi.org/10.1016/S0167-8655(98)00057-9"
            Label(self.thresholdSettingsFrame,text="     No additional settings needed.",fg='red').pack(anchor=W)
        elif self.intSettings['thresholdMethod'].get() == 5:
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"Thresholds the image using fuzzy set theory. The 'index of fuzziness' between the binarized and the gray image is calculated using Shannon's function. The threshold level that minimizes the index of fuzziness is chosen. Performs poorly when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            self.thresholdLink = r"http://dx.doi.org/10.1016/0031-3203(94)E0043-K"
            Label(self.thresholdSettingsFrame,text="     No additional settings needed.",fg='red').pack(anchor=W)
        elif self.intSettings['thresholdMethod'].get() == 6:
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"An iterative method that uses a switching function to classify voxels as either foreground or background. The first iteration defines the switch based on the assumption that the image corners are background and the rest is foreground. The binarized image that results is then used to define the switching function in the next iteration. Performs poorly when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            ...
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"This method approximates the image histogram as two Gaussian distributions estimating the histogram above and below the current threshold level. The threshold that produces minimal overlap between the Gaussian fits is taken. Performs poorly when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            self.thresholdLink = r"http://dx.doi.org/10.1016/0031-3203(86)90030-0"
            Label(self.thresholdSettingsFrame,text="     No additional settings needed.",fg='red').pack(anchor=W)
        elif self.intSettings['thresholdMethod'].get() == 8:
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"This method calculates the moments of the gray image and then determines the threshold level that yields a thresholded image that has the same moments. Performs poorly when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            self.thresholdLink = r"http://dx.doi.org/10.1016/0734-189X(85)90133-1"
            Label(self.thresholdSettingsFrame,text="     No additional settings needed.",fg='red').pack(anchor=W)
        elif self.intSettings['thresholdMethod'].get() == 9:
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"Performs a multilevel threshold that minimizes a cost function based on similarity between the original and thresholded images and the total number of bits needed to represent the thresholded image. The balance of these two terms typically has a unique minimum. This method is much less expensive than entropy based methods, but has shown poor performance when objects are in close proximity.")
            self.textThresholdHelp["state"] = DISABLED
            ...
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"The same as the Maximum Entropy measure, but uses the Renyi entropy definition. In practice this sets the threshold value lower than the Maximum Entropy approach, so is more likely to capture faint voxels. This can cause problems when objects are close.")
            self.textThresholdHelp["state"] = DISABLED
            self.thresholdLink = r"http://dx.doi.org/10.1016/0734-189X(85)90125-2"
            Label(self.thresholdSettingsFrame,text="     No additional settings needed.",fg='red').pack(anchor=W)
            ...
            self.textThresholdHelp["state"] = NORMAL
            self.textThresholdHelp.delete("0.0",END)
            self.textThresholdHelp.insert(END,"A modification of the Maximum Entropy method. This method additionally considers the voxel's 'distance' from the determined threshold in its analytics. This results in a more aggressive thresholding with fewer faint voxels classified as white. For microscopy images, voxels with partial volume effects are more likely to not be considered an object with this approach. This will perform better still than Maximum Entropy when objects are close.")
            self.textThresholdHelp["state"] = DISABLED
    ...
    def run_segmentation(self):
        ...
        for i,d in enumerate(self.directories):
            ...
            if self.intSettings['activeMethod'].get() == 1:
                ...
                vol.geodesicSegmentation(upsampling=int(self.settings['upsampling'].get()),
                ...
                vol.edgeFreeSegmentation(upsampling=int(self.settings['upsampling'].get()),
                                         seed_method=self.thresholdMethods[self.intSettings['thresholdMethod'].get()-1],
                                         ratio=self.settings['thresholdPercentage'].get(),
